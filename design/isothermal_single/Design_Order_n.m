...
%% Case 1: eps = 0 (no changes in number of moles)
eps = 0;
for i=1:length(X)
...
title('eps = 0');   legend('Batch','Plug Flow', 'CSTR');
xlim([0.01 1]);
...
subplot(2,2,2);
loglog(1-X,TauBR, 1-X,TauPFR,'.', 1-X,TauCSTR);
title('eps = 1');   legend('Batch','Plug Flow', 'CSTR');
xlim([0.01 1]);
xlabel('1-X');  ylabel('residence time [s]');
...
subplot(2,2,3);
loglog(1-X,TauBR, 1-X,TauPFR,'.', 1-X,TauCSTR);
title('eps = -0.5');    legend('Batch','Plug Flow', 'CSTR');
xlim([0.01 1]);
xlabel('1-X');  ylabel('residence time [s]');
...
title('CSTR vs PFR');   legend('eps=0','eps=1','eps=-0.5');
xlim([0.01 1]); ylim([0.1 100]);
...
title('Levenspiel''s Plot');    legend('n=1', 'n=2', 'n=0.5');
xlim([0.01 1]); ylim([1 100]);
xlabel('X (Conversion)');   ylabel('-1/R_A [m3.s/kmol]');
