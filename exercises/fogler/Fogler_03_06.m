...
title('Levenspiel Plot');   legend('Batch','Flow');
xlim([0 0.5]);  ylim([0 1000]);
xlabel('conversion');   ylabel('-1/R_A [dm^3min/mol]');

subplot(2,2,2);
hold all;
plot(XBatch,batch_minus_RA,'LineWidth',2);
plot(XFlow,flow_minus_RA,'LineWidth',2);
title('Reaction rate plot'); legend('Batch','Flow');
xlim([0 0.5]);  %ylim([0 1000]);
xlabel('conversion');   ylabel('r [mol/dm^3/min]');

subplot(2,2,3);
hold all;
plot(XBatch,batch_CA,'LineWidth',2);
plot(XFlow,flow_CA,'LineWidth',2);
title('Concentration species A');   legend('Batch','Flow');
xlim([0 0.5]);
xlabel('conversion');   ylabel('CA [mol/dm^3]');

subplot(2,2,4);
hold all;
plot(XBatch,batch_CB,'LineWidth',2);
plot(XFlow,flow_CB,'LineWidth',2);
title('Concentration species B');legend('Batch','Flow');
xlim([0 0.5]);
xlabel('conversion');   ylabel('CB [mol/dm^3]');
