...
function [r,E,MH,resL,resG,resK] = OverallRateOfChange(pA, CB, data)
    ...
    kIII = data.kIII;       % kinetic constant (liquid volume basis) [m6/kmol^2/s]
    
    
    %% Preliminary calculations
    kIIIStar = kIII*CB;                % [m3/kmol/s]
    kIIITilde = kIIIStar*CB;           % [1/s]
    KL = KLa/a;                        % [m/s]
    MH=sqrt(kIIIStar*GammaA*CB)/KL;   % [-]
    resG=1/(KGa);                      % [m3.s.Pa/kmol]
    resK=HA/(kIIITilde*fL);            % [m3.s.Pa/kmol]
