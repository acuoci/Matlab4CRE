...
% Application of a false transient method                                 %
...
% NLS solution                                                            %
% ------------------------------------------------------------------------%
Yin = [CAin CBin CCin Tin]';
options = odeset('RelTol',1e-6, 'AbsTol',1e-8);
[t, Y] = ode15s(@CSTR, [0 10*tau], Yin, options);
...
figure;
plot(t,CA, t,CB, t,CC);
xlabel('time [s]'); ylabel('concentrations [kmol/m3]'); 
title('concentrations'); legend('A', 'B', 'C');
...
function dY = CSTR(t,Y)
    ...
    Ctot = CA+CB+CC;
    Cp = (CA*CpA+CB*CpB+CC*CpC)/Ctot;
    
    % ODE equations
    dCA = (CAin-CA)/tau + RA;
    dCB = (CBin-CB)/tau + RB;
    dCC = (CCin-CC)/tau + RC;
    dT  = (HAin*CAin+HBin*CBin+HCin*CCin - (HA*CA+HB*CB+HC*CC)) ...
           / (tau*Cp*Ctot);
