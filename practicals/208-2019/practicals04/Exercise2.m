...
% Solution of a non linear equation inside the ODE                        %
...
figure;
plot(z,T,'-o', vector_z, vector_Te-273.15,'-o');
...
function dY = PFR(z,Y)
    ...
    Te = fsolve(@HeatExchange, TeFirstGuess, options);
    TeFirstGuess = Te;
...
function F = HeatExchange(x)
    ...
    % Non-linear equation
    F = he*(Tfgas-x) + FSigmaEps*(Tfwall^4-x^4) + Beta*(Tfgas^4-x^4) - ...
        Ue*(x-T);
