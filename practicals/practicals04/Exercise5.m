...
% Pressure equation added                                                 %
% Formation of coke added                                                 %
...
Ctotin = Pin/8314/Tin;  % inlet concentration [kmol/m3]
...
figure;
plot(z,P,'-o');
xlabel('axial coordinate [m]'); ylabel('P [atm]'); 
title('pressure');
...
function dY = PFR(z,Y)
    ...
    dF_C3H8 = -Stilde*(r1+r2);           % [kmol/m/s]
    dF_C2H4 =  Stilde*r1;                % [kmol/m/s]
    dF_CH4  =  Stilde*r1;                % [kmol/m/s]
    dF_C3H6 =  Stilde*r2;                % [kmol/m/s]
    dF_H2   =  Stilde*r2;                % [kmol/m/s]
    dF_H2O  =  0;                        % [kmol/m/s]
       
    % Calculation of internal heat echange coefficient
    Qtot = Ftot/Ctot;                   % [m3/s]
    v = Qtot/Stilde;                    % [m/s]
    rho = mtot/(v*Stilde);              % [kg/m3]
    Re = rho*v*di/muMix;                % [-]
    ...
    if (Di>di)
        Ue = 1/( De/di/hi + ...
                 (s/lambdaWall)*(De*log(De/Di))/(De-Di) + ...
                 (scoke/lambdaCoke)*(De*log(Di/di))/(Di-di) );   % [kcal/m2/s/K]
    else
        Ue = 1/( De/di/hi + ...
    ...
    dT  = -Stilde*( Ui*(T-Te)*4/di + QR)/(mtot*CpMix);  % [K/m]
    
    % Find external wall temperature
    dTe = he*(Tfgas-Te) + FSigmaEps*(Tfwall^4-Te^4) + Beta*(Tfgas^4-Te^4) -...
          Ue*(Te-T);   
      
    % Pressure equation
    f = 0.079/Re^0.25;
    dP = -0.5*rho*v^2*f*4/di;

    % Recover derivatives in time
    dY = [dF_C3H8 dF_C2H4 dF_CH4 dF_C3H6 dF_H2 dF_H2O dT dTe dP]';
