...
% y = a0 + a1*x1 + a2*x2 - log(1+b2*PH2^nH2high)
 
%% Non-linear regression model 

% First guess
firstGuess = [-5 1 0.01 1 1];
...
nCO     = nlm.Coefficients.Estimate(2);
nH2low  = nlm.Coefficients.Estimate(3);
nH2high = nlm.Coefficients.Estimate(4);
b2      = nlm.Coefficients.Estimate(5);

% Quality evaluation
rCH4model = b1*(PCO.^nCO).*(PH2.^nH2low) ./ (1+b2*PH2.^nH2high);
SSres = sum( (rCH4-rCH4model).^2  );
SStot = sum( (rCH4-mean(rCH4)).^2 );
R2 = 1 - SSres/SStot;
...
function y = nonLinearModel(a,x)
    ...
    y = lnb1 + nCO*log(PCO) + nH2low*log(PH2) - log(1+b2*PH2.^nH2high);
